...
public class MyGL20Renderer implements MyGLSurfaceView.Renderer {
    ...
    public void onDrawFrame(GL10 unused) {
       ...
       // Matrix.setLookAtM(mVMatrix, 0, 0, 0, -3, 0f, 0f, 0f, 0f, 1.0f, 0.0f);
        ...
        for (float x = -4;x<4;x++) {
        	for(float y = -4;y<4;y++) {
                if(backend_chess.board[(int) (x+4)][(int) (y+4)] != 0) {
                	centreCoords[0] = y * 0.25f+0.125f;
                	centreCoords[1] = x * 0.25f+0.125f;
                	if (backend_chess.getIndex((int)x+4, (int)y+4) == squareStart) {
                		centreCoords[0] = startX;
                    	centreCoords[1] = startY;
                	}
                    if (pieceCount < 32) {
                        cPiece[pieceCount] = backend_chess.board[(int) (x + 4)][(int) (y + 4)];
        ...
        if (backend_chess.GAME_THREATS) {
        	tmp = backend_chess.getThreats(colour);
        	j = 0;
        	while (j < tmp[0][0]) {
        		if (tmp[j+1][1] == 0) {
		        	//Threats -> ORANGE
        ...
        if (squareStart != -1 && backend_chess.GAME_OPTIONS) {
        	tmp = backend_chess.getOptions(colour, squareStart);
        	j = 0;
        	while (j < tmp[0][0]) {
        		if (tmp[j+1][1] == 0) {
        ...
        for(int i = 0;i<32;i++) {
        	if(cPiece[i] <= backend_chess.BLACK_KING) {
...
class Piece {
	...
	public float[] getPieceCoords(float centreCoord[], int piece, int colour) {
		float pieceCoord[];
		if (piece == backend_chess.WHITE_PAWN || piece == backend_chess.BLACK_PAWN) {
		...
		} else if (piece == backend_chess.WHITE_ROOK || piece == backend_chess.BLACK_ROOK) {
		...
		} else if (piece == backend_chess.WHITE_KNIGHT || piece == backend_chess.BLACK_KNIGHT) {
		...
		} else if (piece == backend_chess.WHITE_BISHOP || piece == backend_chess.BLACK_BISHOP) {
		...
		} else if (piece == backend_chess.WHITE_QUEEN || piece == backend_chess.BLACK_QUEEN) {
