...
public class Chess {
	...
	public static int PROBABILISTIC = 2;
	public static int GAME_TYPE = PROBABILISTIC;

	public static int HINTS_ON = 1;
	public static int HINTS_OFF = 2;
	public static int GAME_HINTS = HINTS_ON;
	...
	static void setGameActivity(GameActivity gameActivity) {
		game = gameActivity;
	}
	...
	public static boolean move (int colour, String string) {
		int curx = 0;
		int cury = 0;
		int newx = 0;
		int newy = 0;
	    ...
	    if (str.countTokens() == 4) {
			curx = Integer.parseInt(str.nextToken()) - 1;
			cury = Integer.parseInt(str.nextToken()) - 1;
			newx = Integer.parseInt(str.nextToken()) - 1;
			newy = Integer.parseInt(str.nextToken()) - 1;
		...
		if (x >= 0 && x <8 && y >= 0 && y <8) {
			if (board[x][y]> 0) {
				if (colour == colour(x,y)) {
					...
					//game.printToConsole("Not your piece at colour: "+colour+", x: " + x + ", y: " + y);
				}
			} else {
				//game.printToConsole("No piece located at x: " + x + ", y: " + y);
			}
		} else {
			//game.printToConsole("Out of bounds at x: " + x + ", y: " + y);
		}
		return status;
	}
	...
	public static void commitMove(int piece, int colour, int curx, int cury, int newx, int newy) {
		boolean status = true;		
		String message;
		if (GAME_MODE == ONE_PLAYER && colour == ChessEngine.computerColour) {
			//Keep screen
		} else {
			game.flushConsole();
		}
		if (colour(newx,newy) != 0 && colour(newx,newy) != colour && GAME_TYPE == PROBABILISTIC) {
			status = false;
	...
	public static void printList() {
		int i;
		for (i = 0; i < 32; i++) {
			lookupPiece(list[i][0]);
			game.printToConsole(" is at " + list[i][1] + "," + list[i][2]);
		}	
		game.printToConsole("\n\n");
	}
